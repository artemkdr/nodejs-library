...
type MetadataFilter = Record<
    ...
    | {
          ...
          arrayContainsAll?: (string | number | boolean)[];
...
export class PGVectorStore extends VectorStore {
    ...
    constructor(embeddings: EmbeddingsInterface, config: PGVectorStoreArgs) {
        ...
        const pool =
            config.pool ?? new pg.Pool(config.postgresConnectionOptions);
        this.pool = pool;
        this.chunkSize = config.chunkSize ?? 500;
        if (config.distanceStrategy)
            this.distanceStrategy =
                config.distanceStrategy ?? this.distanceStrategy;
    ...
    get computedOperatorString() {
        ...
        switch (this.distanceStrategy) {
            ...
            default:
                throw new Error(
                    `Unknown distance strategy: ${this.distanceStrategy}`,
                );
        }
    ...
    private buildFilterClauses(filter: MetadataFilter, paramOffset = 0) {
        ...
        for (const [key, value] of Object.entries(filter)) {
            if (typeof value === "object" && value !== null) {
                const _value: Record<string, unknown> = value as Record<
                    string,
                    unknown
                >;
                ...
                if (Array.isArray(_value["arrayContainsAll"])) {
                    const placeholders = _value["arrayContainsAll"]
                        .map(
                            (_: unknown, index: number) =>
                                `$${currentParamCount + index + 1}`,
                        )
                    ...
                    parameters.push(..._value["arrayContainsAll"]);
                    paramCount += _value["arrayContainsAll"].length;
                }
    ...
    private async searchPostgres(
      ...
      `;
        ...
        const documents = (await this.pool.query(queryString, allParameters))
            .rows;
        ...
        for (const doc of documents) {
            if (doc._distance != null && doc[this.contentColumnName] != null) {
                ...
                if (includeEmbedding) {
                    document.metadata[this.vectorColumnName] =
                        doc[this.vectorColumnName];
                }
    ...
    static override async fromTexts(
        ...
        for (let i = 0; i < texts.length; i += 1) {
            const text = texts[i];
            if (!text) {
                continue; // Skip undefined or empty texts
            }
    ...
    static override async fromDocuments(
        ...
        const options = dbConfig.ids ? { ids: dbConfig.ids } : undefined;
    ...
    async createHnswIndex(config: {
        ...
        let idxDistanceFunction =
            config?.distanceFunction || "vector_cosine_ops";
    ...
    override async maxMarginalRelevanceSearch(
        ...
        return mmrDocs.filter((doc): doc is Document => doc !== undefined);
    }
}
