...
describe("Logger with Sensitive Data Masking", () => {
	...
	describe("warn logging with masking", () => {
		...
		it("should mask sensitive data in warning metadata", () => {
			...
			expect(mockWarn).toHaveBeenCalledWith("Deprecated API endpoint used", {
				endpoint: "/v1/users",
				clientId: "client123",
				apiKey: "***MASKED***",
			});
	...
	describe("debug logging with masking", () => {
		...
		it("should mask arrays in debug metadata", () => {
			...
			expect(mockDebug).toHaveBeenCalledWith("Processing batch requests", {
				requests: [
	...
	describe("edge cases", () => {
		...
		it("should handle null and undefined metadata gracefully", () => {
			logger.error("Error message", null as unknown as Record<string, unknown>);
		...
		it("should handle non-object metadata gracefully", () => {
			...
			expect(mockInfo).toHaveBeenCalledWith("Info message", "string metadata");

			logger.debug("Debug message", 123 as unknown as Record<string, unknown>);
	...
	describe("real-world scenarios", () => {
		...
		it("should mask API integration logs", () => {
			...
			expect(mockInfo).toHaveBeenCalledWith("External API call completed", {
				url: "https://api.service.com/v1/data",
				method: "POST",
				headers: {
					"Content-Type": "application/json",
	...
	describe("format configuration", () => {
		...
		it("should default to json format for unknown format", () => {
			const unknownConfig = {
				...config,
				format: "unknown" as unknown as "json" | "simple" | "combined",
			};
			...
			const mockCreateLogger = mock().mockImplementation((config) => {
				...
				if (config.format.format) {
					...
					expect(result1).toContain(
						"2025-09-20T10:00:00.000Z [INFO]: Test message",
					);
	...
	describe("integration scenarios", () => {
		...
		it("should handle minimal configuration", () => {
			...
			expect(mockError).toHaveBeenCalledWith("Minimal error", {
				sensitive: "data",
			});
		});
	});
